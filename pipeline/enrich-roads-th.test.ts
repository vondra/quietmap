...
test('DRR exact ref wins before multi-ref motorway and Bangkok trunk policy', () => {
  ...
  assert.deepEqual(matchThailandRoad(road('7;9'), source),
    { ...roadObservation('7;9', 'both-directions'), kind: 'drr', light: 2, medium: 1, heavy: 1, moto: 1 })
  ...
  // A motorway_link tagged with the mainline ref takes neither the DRR count nor the route policy.
  assert.equal(matchThailandRoad({ ...road('7;9'), roadClass: 10 }, source), null)
})
