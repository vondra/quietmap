...
test('national ownership includes the declared Western Sahara extension for Morocco only', () => {
  assert.deepEqual(countryIsosForNationalSource(9505), ['MA', 'EH'])
  assert.deepEqual(countryIsosForNationalSource(9181), ['CD'])
  assert.equal(countryIsosForNationalSource(10), null)
})
