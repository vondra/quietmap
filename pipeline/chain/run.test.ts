...
test('command argv never imports enrichers and uses enrich-only on national roads', () => {
  ...
  const py = commandFor(buildPlan({ kind: 'world' }).find(step => step.id === 'roads-py')!, paths)
  assert.ok(py.argv.some(arg => arg.endsWith('enrich-roads-national-network.ts')))
  assert.deepEqual(py.argv.slice(-2), ['--country', 'py'])
  assert.ok(py.argv.includes('--enrich-only'))
