...
const line = (properties: Record<string, unknown>, relativePath = 'pe/roads-nacional-mtc.geojson'): PinnedRoadLine => ({
  observationId: 'fixture', coordinates: [[-77, -12], [-76.99, -12]], properties, relativePath,
})
...
test('Peru keeps dIMD unscaled inside Lima, before classification', () => {
  assert.deepEqual(matchPeruRoad(road, source([line({ dIMD: 1000, cSuperfici: '1', cClasifica: 'TRANSVERSAL' })])),
    { countBasis: 'both-directions', observationId: 'fixture', kind: 'imd', light: 650, medium: 60, heavy: 140, moto: 150 })
})
...
test('Peru retains departmental paved fallback and rejects lower OSM classes', () => {
  ...
  assert.deepEqual(matchPeruRoad(road, departmental),
    { countBasis: 'both-directions', observationId: 'fixture', kind: 'network', light: 3250, medium: 300, heavy: 700, moto: 750 })
