...
export function squareSamples(preparedDirectory: string, square: string): Float64Array {
  ...
  const [classes, oneway, junction, lanes, builtUp, access, sources, lengths] =
    ['road_class', 'oneway', 'junction', 'lanes', 'built_up', 'access', 'source_id', 'length_m'].map(column)
  ...
  for (let row = 0; row < table.numRows; row++) {
    ...
    if (classes[row] >= PRIOR_CLASSES || !COUNTED_SOURCES.has(sources[row]) || junction[row] !== 0 || access[row] !== 0 ||
        tunnel.get(row) || !(count > 0) || !(lengths[row] > 0)) continue
