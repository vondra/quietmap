...
export async function enrichMunicipalRoads(preparedDirectory: string, cities: readonly AdmittedCity[]) {
  ...
  for (const plan of plans) {
    ...
    const toAadt = (record: CityRoadRecord | null) => record ? { countBasis: record.countBasis, observationId: record.observationId, light: record.light, medium: record.medium, heavy: record.heavy, moto: record.moto, sourceId: plan.city.sourceId, estimatedClasses: record.estimatedClasses } : null
