 ...
 * by `enrichRailwaysByGraphWalk`. TRAM/light-rail (rail_type 1/2 — Letbane,
 * Metro) keeps the pre-Phase-4 `nearestGridStop` 500 m stop-join
 ...
 *
 * DK's source id is NATIONALLY OWNED (unlike europe's shared
 * `SOURCE_ID_GLOBAL_GTFS_TRANSIT`): `bleedGate` is set to the SAME country
 * gate as `countryGate` — a row wholly outside Denmark is disownable on
 * geometry alone, since this id never legitimately stamps outside DK. No
 * predecessor source id survives from the pre-2026-07-10 class-default
 * fallback design (DK only ever used ONE id): the driver's own generic retract
 ...
 * unknowns; DK is not silent-residual-capable (that exists only for CZ's
 ...
 *   DATA_YEAR=2026 npx tsx pipeline/enrich-railway-dk.ts --stamp-only
 *
 ...
 *  cross-border stop's pair could land in, or a DK-DE/SE through-train's
 ...
 *  `computeStopPairFrequenciesForFeed`) gets the UNPADDED `DK_BBOX` — the
 *  margin is applied internally by `loadStopsWithCoords`. */
const GRAPH_BBOX: [number, number, number, number] = [
  DK_BBOX[0] - GTFS_BORDER_MARGIN_DEG, DK_BBOX[1] - GTFS_BORDER_MARGIN_DEG,
  DK_BBOX[2] + GTFS_BORDER_MARGIN_DEG, DK_BBOX[3] + GTFS_BORDER_MARGIN_DEG,
]

// ── Main ──

async function main() {
  console.log(`=== DK Railway Enrichment — Rejseplanen unified GTFS (${YEAR}, Phase 4: graph-walk) ===\n`)
  ...
  for (const { feed, dir } of feeds) {
    try {
      ...
      const { pairs } = await computeStopPairFrequenciesForFeed(dir, { bbox: DK_BBOX })
  ...
  // (Rejseplanen also covers a handful of Sweden/Germany border routes), so this
  // id must not speak for track outside Denmark — same rationale as every other
  ...
  const stats = await enrichRailwaysByGraphWalk({
    ...
    // the SAME gate doubles as the bleed arm — a row wholly outside Denmark is
    ...
    // Single id — DK has no retired predecessor id to absorb. The pre-2026-07-10
    ...
    // #26B: DK is NOT silent-capable (silentResidual omitted) — a
    ...
    sidecar: {
      scope: 'dk',
      extractFingerprint: `dk-gtfs:${feeds.map(({ feed }) => feed.id).join('+')}`,
