 ...
 * (national rail incl. DART), Luas (Dublin tram), Dublin Bus, Bus Éireann,
 * Go-Ahead. Single 167 MB zip, updated daily.
 *
 ...
 * by `enrichRailwaysByGraphWalk`. TRAM (rail_type 1/2 — Luas) keeps the
 * pre-Phase-4 `nearestGridStop` 500 m stop-join (`buildTramExtraMatch`,
 * hoisted to `lib/gtfs-enrich-core.ts` so this file shares the SAME
 * implementation every other national enricher uses), wired in as the walk
 * driver's `extraMatch` fallback arm.
 *
 * IE's source id is NATIONALLY OWNED (unlike europe's shared
 * `SOURCE_ID_GLOBAL_GTFS_TRANSIT`): `bleedGate` is set to the SAME country
 * gate as `countryGate` — a row wholly outside Ireland is disownable on
 * geometry alone, since this id never legitimately stamps outside IE (the
 * NTA feed's cross-border Enterprise/Translink legs into Northern Ireland
 * fall outside `countryGate` and are simply never walked/retracted by this
 * run — Northern Ireland's own track is GB's business, not this file's). No
 * predecessor source id survives from the pre-2026-07-10 class-default
 * fallback design (IE only ever used ONE id): the driver's own generic
 * retract (disown a previously-owned row the walk/tram-join no longer
 * covers, given `retractSafe`) already subsumes the old OLD_FALLBACK
 * exact-tuple check, so that class-default table is deleted outright rather
 ...
 * unknowns; IE is not silent-residual-capable (that exists only for CZ's
 * timetable, owner decision 2026-07-11 option b).
 *
 * Usage:
 *   DATA_YEAR=2026 npx tsx pipeline/enrich-railway-ie.ts
 *   DATA_YEAR=2026 npx tsx pipeline/enrich-railway-ie.ts --force-download
 *   DATA_YEAR=2026 npx tsx pipeline/enrich-railway-ie.ts --enrich-only
 *   DATA_YEAR=2026 npx tsx pipeline/enrich-railway-ie.ts --stamp-only
 *
...
const FEEDS: FeedConfig[] = [
  {
    id: 'nta-unified',
    name: 'NTA Transport for Ireland unified (Iarnród Éireann + DART + Luas + buses)',
    urls: [
...
async function downloadAllGtfs(): Promise<Array<{ feed: FeedConfig; dir: string }>> {
  ...
  if (results.length === 0) {
    throw new Error('Failed to download any Irish GTFS feed')
  }

  console.log(`  ${results.length}/${FEEDS.length} IE feeds available`)
  return results
}
 ...
 *  cross-border stop's pair could land in, or a IE-NI through-train's
 ...
 *  `computeStopPairFrequenciesForFeed`) gets the UNPADDED `IE_BBOX` — the
 *  margin is applied internally by `loadStopsWithCoords`. */
const GRAPH_BBOX: [number, number, number, number] = [
  IE_BBOX[0] - GTFS_BORDER_MARGIN_DEG, IE_BBOX[1] - GTFS_BORDER_MARGIN_DEG,
  IE_BBOX[2] + GTFS_BORDER_MARGIN_DEG, IE_BBOX[3] + GTFS_BORDER_MARGIN_DEG,
]

// ── Main ──

async function main() {
  console.log(`=== IE Railway Enrichment — NTA unified GTFS (${YEAR}, Phase 4: graph-walk) ===\n`)
  ...
  for (const { feed, dir } of feeds) {
    try {
      ...
      const { pairs } = await computeStopPairFrequenciesForFeed(dir, { bbox: IE_BBOX })
      perFeedPairs.push(pairs)
      let tramN: number | null = null
      if (!stopCacheHit) {
        const counts = await computeStopFrequenciesForFeed(feed, dir, IE_BBOX)
  ...
  // (NTA's unified GTFS also covers the cross-border Enterprise/Translink legs into
  // Northern Ireland), so this id must not speak for track outside Ireland — same
  // rationale as every other national enricher (mechanism: the PL feed once stamped
  // 11,856 km of CZ track, 7fac2349). #31.7 central country gate — see
  // writeRailTrains / rail-walk-enrich.ts.
  const inIe = makeCountryGate('IE')
  ...
  const stats = await enrichRailwaysByGraphWalk({
    ...
    // the SAME gate doubles as the bleed arm — a row wholly outside Ireland is
    ...
    // Single id — IE has no retired predecessor id to absorb. The pre-2026-07-10
    ...
    // #26B: IE is NOT silent-capable (silentResidual omitted) — a
    ...
    sidecar: {
      scope: 'ie',
      extractFingerprint: `ie-gtfs:${feeds.map(({ feed }) => feed.id).join('+')}`,
