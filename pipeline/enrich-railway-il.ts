 ...
 * by `enrichRailwaysByGraphWalk`. TRAM/light-rail (rail_type 1/2 — Tel Aviv
 * LRT, Jerusalem LRT) keeps the pre-Phase-4 `nearestGridStop` 500 m stop-join
 * (`buildTramExtraMatch`, hoisted to `lib/gtfs-enrich-core.ts` so this file
 * shares the SAME implementation every other national enricher uses), wired
 * in as the walk driver's `extraMatch` fallback arm.
 *
 * IL's source id is NATIONALLY OWNED (unlike europe's shared
 * `SOURCE_ID_GLOBAL_GTFS_TRANSIT`): `bleedGate` is set to the SAME country
 * gate as `countryGate` — a row wholly outside Israel is disownable on
 * geometry alone, since this id never legitimately stamps outside IL. No
 * predecessor source id survives from the pre-2026-07-10 class-default
 * fallback design (IL only ever used ONE id): the driver's own generic
 ...
 * unknowns; IL is not silent-residual-capable (that exists only for CZ's
 ...
 *   DATA_YEAR=2026 npx tsx pipeline/enrich-railway-il.ts --stamp-only
 *
...
// Israel bbox
const IL_BBOX: [number, number, number, number] = [29.5, 34.2, 33.4, 35.9]
 ...
 *  the UNPADDED `IL_BBOX` — the margin is applied internally by
 *  `loadStopsWithCoords`. */
const GRAPH_BBOX: [number, number, number, number] = [
  IL_BBOX[0] - GTFS_BORDER_MARGIN_DEG, IL_BBOX[1] - GTFS_BORDER_MARGIN_DEG,
  IL_BBOX[2] + GTFS_BORDER_MARGIN_DEG, IL_BBOX[3] + GTFS_BORDER_MARGIN_DEG,
]

// ── Main ──

async function main() {
  console.log(`=== IL Railway Enrichment — MoT unified GTFS (${YEAR}, Phase 4: graph-walk) ===\n`)
  ...
  for (const { feed, dir } of feeds) {
    try {
      ...
      const { pairs } = await computeStopPairFrequenciesForFeed(dir, { bbox: IL_BBOX })
      perFeedPairs.push(pairs)
      let tramN: number | null = null
      if (!stopCacheHit) {
        const counts = await computeStopFrequenciesForFeed(feed, dir, IL_BBOX)
  ...
  // so this id must not speak for track outside Israel — same rationale as every
  ...
  const stats = await enrichRailwaysByGraphWalk({
    ...
    // the SAME gate doubles as the bleed arm — a row wholly outside Israel is
    ...
    // Single id — IL has no retired predecessor id to absorb. The pre-2026-07-10
    ...
    // #26B: IL is NOT silent-capable (silentResidual omitted) — a
    ...
    sidecar: {
      scope: 'il',
      extractFingerprint: `il-gtfs:${feeds.map(({ feed }) => feed.id).join('+')}`,
