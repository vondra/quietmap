...
export class NoiseOnflySupervisor {
   ...
   * rayon threads and mimalloc heaps were alive, and the next spawn's re-map
   * crashed the server (SIGSEGV twice on 2026-09-06). The late reply frees the
   * slot like any other reply (and still fills the point cache); a worker that
   * really dies goes through the error/exit path, the only recycle triggers.
   */
