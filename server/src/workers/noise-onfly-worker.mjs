...
// per-threadId copies made every recycle (worker error or exit)
...
// distinct-path variant is what exhausted TLS live in production). The
...
// Rust square cache and sidecar cache are shared across the pool (an area
...
if (existsSync(preparedYearDir)) {
  const msg = sourceModule.sourceInit(preparedYearDir)
...
parentPort?.on('message', ({ id, lat, lng, lat2, lng2, op }) => {
  try {
    if (op === 'ready') {
      ...
      sourceModule.sourceInit(preparedYearDir)
      const referenceRows = sourceModule.sourceValidateReference(
        preparedYearDir,
        readinessReferenceSquare,
      )
