---
title: The Bahamas
intro: Roads use OSM class defaults scaled by 1.261. No railway. Power plants from Global Energy Monitor.
map: { center: [-77.5, 24.6], zoom: 7 }
---

## Roads

No open traffic counts; roads use OSM class defaults. Motorways, trunks, primaries and their ramps are scaled by 1.261 (registered vehicles per road kilometre relative to Germany).

| OSM class | Default vehicles per day |
|---|---:|
| Motorway | 30,000 × 1.261 = 37,830 |
| Trunk | 15,000 × 1.261 = 18,915 |
| Primary | 9,000 × 1.261 = 11,349 |
| Secondary | 3,000 |
| Tertiary | 800 |
| Residential | 500 |
| Unclassified | 1,340 |
| Service | 250 |
| Track | 5 |

Where buildings are mapped, residential, living, service and unclassified streets are instead estimated from the buildings they serve.

## Railways

The Bahamas has no railway.

## Industry

Power plants: Global Energy Monitor, operating plants only, where OSM maps an industrial area at the site. Other industry: OSM industrial areas with a type inferred from name and tags.

## Ships

Global Fishing Watch AIS vessel density.
