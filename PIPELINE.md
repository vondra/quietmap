...
dataset once, then re-spawn their own command line per shard; each shard reloads the dataset, so the
shard count is bounded by memory divided by the loaded parent's resident size plus one square.
