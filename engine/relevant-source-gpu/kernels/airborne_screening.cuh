...
struct AirborneScreen {
    const unsigned int* terrain;
    const float* terrain_max;
    const unsigned int* buildings;
    const unsigned short* building_max;
    const unsigned short* global_max;
    unsigned int records;
};
static_assert(sizeof(AirborneScreen) == 48);
...
__device__ __forceinline__ float terrain_screening_db(
    const AirborneScreen& screen, unsigned int record,
    float cpa_east_m, float cpa_north_m, float rel_alt_m, float slant_sq)
{
    unsigned long long records = screen.records;
    const float* max_sin_sq = screen.terrain_max;
...
__device__ __forceinline__ float receiver_screening_db(
    const AirborneScreen& screen, int pixel,
    float cpa_east_m, float cpa_north_m, float rel_alt_m, float slant_sq,
    float source_east_m, float source_north_m, float source_rel_alt_m)
{
    unsigned int record = pixel;
