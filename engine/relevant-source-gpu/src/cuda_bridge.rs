...
unsafe extern "C" {
    ...
    fn relevant_source_cuda_evaluate_corners(
        ...
        corner_count: u32,
        corner_pixel_floor_m: *const f32,
    ...
    fn relevant_source_cuda_paint_tile(
        scene: *const DeviceScenePointers,
        receiver_pixel_floor_m: *const f32,
...
pub trait DeviceValue: Copy + device_value::Sealed {}
impl<T: Copy + device_value::Sealed> DeviceValue for T {}
...
impl<T: DeviceValue> DeviceBuffer<T> {
    ...
    pub fn uninitialized(length: usize) -> Result<Self> {
        let bytes = length
            ...
            .ok_or_else(|| anyhow::anyhow!("CUDA allocation length overflow"))?;
        let mut pointer = std::ptr::null_mut();
        check_cuda(unsafe { relevant_source_cuda_allocate(&mut pointer, bytes) })?;
        Ok(Self {
...
impl RelevantSourceCuda {
    ...
    pub fn evaluate_corners(
        ...
        corner_pixel_floor_m: &DeviceBuffer<f32>,
        ...
        if corner_count == 0
            ...
            || corner_offsets.element_count() != corner_count + 1
    ...
    pub fn paint_tile(
        &self,
        scene: &DeviceScenePointers,
        receiver_pixel_floor_m: &DeviceBuffer<f32>,
        ...
        if receiver_x_m.element_count() != pixel_count
            ...
            || background_energy.element_count()
                != crate::source_frame::BLOCK_COUNT * 4 * PERIOD_COUNT
        {
