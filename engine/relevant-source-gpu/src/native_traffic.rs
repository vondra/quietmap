//! Raw airport event energy becomes mean period power using its file's sampling window.
use super::*;
use arrow::array::{FixedSizeListArray, Float32Array};
use noise_compute::emission::aircraft::{
    ProvenanceWeights, SamplingWindow, GROUND_OPS_SOURCE_HEIGHT_M, NUM_CLASSES, PERIOD_SECONDS,
};
...
impl TrafficCalendar {
    pub(super) fn read(batch: &RecordBatch) -> Result<Self> {
        ...
        let window =
            SamplingWindow::from_metadata(batch.schema().metadata()).map_err(anyhow::Error::msg)?;
        Ok(Self {
...
pub(super) fn traffic_row(
    ...
    for name in ["period", "veh_kind", "class_idx", "ops_kind", "secondary_only"] {
        ensure!(
    ...
    let weight = calendar
        .weights
        .for_secondary_only(byte(batch, "secondary_only", row) != 0);
...
mod tests {
    ...
    fn even_empty_airport_files_require_current_contract_and_valid_calendar() {
        ...
        metadata.insert("baseline_days".into(), "0".into());
