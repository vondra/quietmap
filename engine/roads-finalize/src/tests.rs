...
fn road(id: i64, basis: u8, reverse: bool) -> Road {
    Road {
        way_id: id,
        segment_idx: 0,
        start: (if reverse { 10.0 } else { 0.0 }, 0.0),
        end: (if reverse { 10.0 } else { 0.0 }, 100.0),
        direction: if reverse { 2 } else { 1 },
        class: 2,
        lanes: 2,
        access: 0,
        tunnel: false,
        country: SquareCountryCity::UNKNOWN,
        source_id: 10,
        observation_source_id: 10,
        provenance: provenance_of(10),
        counts: [10_000.0, 0.0, 0.0, 0.0],
        basis,
        estimated: 0,
        observation: "counter:A".to_owned(),
        corridor: "R1".to_owned(),
    }
}
...
fn directional_counts_never_change_with_osm_direction_or_other_carriageways() {
    ...
    for direction in 0..=2 {
        ...
        assert_eq!(
            allocation::resolve(&observed, [&other]),
...
fn section_total_is_shared_once_while_longitudinal_subdivisions_keep_through_flow() {
    ...
    assert_eq!(
        allocation::resolve(&reverse, [&first, &second]),
        whole_reverse
    );
    assert_eq!(allocation::resolve(&first, [&reverse]), whole_forward);
    assert_eq!(allocation::resolve(&second, [&reverse]), whole_forward);
    let already_allocated = Road {
        basis: 3,
        counts: whole_forward.0,
        estimated: 15,
        ..forward
    };
    assert_eq!(
        allocation::resolve(&already_allocated, [&reverse]),
        whole_forward
    );
}
...
fn a_cut_within_a_metre_of_a_parent_end_or_of_another_cut_leaves_no_sliver_child() {
    ...
    assert_eq!(
        allocation::intervals(&long, &[&nearly_whole]),
        [(0.0, 1.0, [5000.0, 0.0, 0.0, 0.0], 15)]
    );
    let staggered = Road {
        class: 3,
        start: (10.0, 0.3),
    ...
    assert_eq!(
        children
            .iter()
            .map(|child| (child.0, child.1, child.2[0]))
            .collect::<Vec<_>>(),
    ...
    let shorter_than_a_metre = Road {
        end: (0.0, 0.8),
        ..long.clone()
    };
    assert_eq!(
        allocation::intervals(&shorter_than_a_metre, &[&staggered]).len(),
        1
    );
}
...
fn staggered_carriageway_ends_split_the_longer_source_and_conserve_each_cross_section() {
    ...
    assert_eq!(
        allocation::intervals(&main_long, &[&main_short]),
        [(0.0, 1.0, [5000.0, 0.0, 0.0, 0.0], 15)]
    );
}
...
fn more_than_two_alternatives_form_one_cross_section_instead_of_pairwise_divisors() {
    ...
    assert!(totals
        .iter()
        .all(|total| (*total - 10_000.0 / 3.0).abs() < 1e-9));
}
...
fn main_class_prior_is_per_carriageway_while_hand_set_section_totals_are_shared() {
    ...
    let total = |road: &Road, others: &[&Road]| {
        allocation::resolve(road, others.iter().copied())
            .0
            .iter()
            .sum::<f64>()
    };
    ...
    assert!(
        (total(&a, &[&b]) - 3.0 * 6379.0).abs() < 1e-8,
        "a matched sibling never divides a carriageway prior"
    );
    assert!((total(&b, &[&a]) - 2.0 * 6379.0).abs() < 1e-8);
    let brazil = SquareCountryCity {
        country_iso: *b"BR",
    ...
    assert!(
        count.iter().any(|v| *v > 0.0 && *v < 1.0),
        "fractional quiet-road priors survive"
    );
}
...
fn dateline_crossing_ipc_splits_along_the_short_wrapped_segment() {
    ...
    for road in [&a, &b] {
        assert_eq!(
            allocation::resolve(road, index.alternatives(road)).0[0],
            5000.0
        );
    }
}
...
fn cross_owner_ipc(dateline: bool) {
    ...
    for (way, x, square, direction) in
        [(1_i64, -5.0, "z9/255/255", 1_u8), (2, 5.0, "z9/256/255", 2)]
    {
        ...
        let coordinate = |x, y| {
            if dateline {
                let (gx, gy) =
                    grid::meters_to_grid(grid::EARTH_CIRCUMFERENCE_M / 2.0 - 20.0 + y, x);
                (gx.rem_euclid(1 << 30), gy)
            } else {
                grid::meters_to_grid(x, y)
            }
        };
        ...
        let mut columns: Vec<(&str, ArrayRef)> = vec![
            ...
            (
                ...
                Arc::new(StringArray::from(vec!["counter:A"])),
            ),
        ];
        for (i, name) in crate::input::COUNTS.iter().enumerate() {
            columns.push((
                name,
                Arc::new(Float64Array::from(vec![if i == 0 {
                    10_000.0
                } else {
                    0.0
                }])),
        ...
        let schema = Arc::new(
            ...
            .with_metadata(std::collections::HashMap::from([(
                "osm_roads_contract".into(),
                square_store::osm_contract::ROADS_CONTRACT.into(),
            )])),
        );
    ...
    assert!(year
        .join(".roads-finalize/z9/255/255/roads.arrow")
        .is_file());
    ...
    let serial_output = paths
        .iter()
        .map(|path| crate::input::load(path).unwrap())
        .collect::<Vec<_>>();
    ...
    for (path, expected) in paths.iter().zip(serial_output) {
        assert_eq!(
            crate::input::load(path).unwrap(),
            expected,
            "parallel allocation must match serial batches and values"
        );
    }
    for (index, path) in paths.iter().enumerate() {
        ...
        assert_eq!(
            batch.column_by_name("aadt_light").unwrap().data_type(),
            &DataType::Float64
        );
        let mut rows = batches
            .iter()
            .flat_map(|batch| crate::input::roads(batch).unwrap())
            .collect::<Vec<_>>();
        if dateline {
            for row in &mut rows {
                let shift = ((grid::EARTH_CIRCUMFERENCE_M / 2.0 - row.midpoint().0)
            ...
            rows.sort_by(|a, b| a.start.1.total_cmp(&b.start.1));
        }
        if index == 0 {
            ...
            assert!(
                (rows[0].end.0 - rows[1].start.0).hypot(rows[0].end.1 - rows[1].start.1) < 1e-6
            );
        } else {
            assert_eq!(rows[0].counts, [5000.0, 0.0, 0.0, 0.0]);
        }
    ...
    assert_eq!(
        crate::finalize_year(&year).unwrap(),
        1,
        "resume partially promoted generation"
    );
    assert_eq!(
        paths
            .iter()
...
fn dense_owner_uses_its_own_batch_without_serializing_sparse_owners() {
    ...
    assert_eq!(first, 1);
    assert_eq!(
        crate::scheduling::batch_end(&growing_budget, first, 3, 16).unwrap(),
        4
    );
    ...
    fn traffic_profile_reference_survives_reblocking() {
        let columns: Vec<(&str, DataType, ArrayRef)> = vec![
            ...
            (
                "start_gx",
                DataType::Int32,
                Arc::new(Int32Array::from(vec![100_000_000])),
            ),
            (
                "start_gy",
                DataType::Int32,
            ...
            (
                "end_gx",
                DataType::Int32,
                Arc::new(Int32Array::from(vec![100_010_000])),
            ),
            (
                "end_gy",
                DataType::Int32,
            ...
            (
                "length_m",
                DataType::Float32,
                Arc::new(Float32Array::from(vec![500.0])),
            ),
            (
                "oneway",
                DataType::UInt8,
            ...
            (
                "ref",
                DataType::Utf8,
                Arc::new(StringArray::from(vec!["B 31"])),
            ),
            (
                "name",
                DataType::Utf8,
        ...
        let schema = Arc::new(
            Schema::new(
                columns
                    .iter()
                    .map(|(name, ty, _)| Field::new(*name, ty.clone(), false))
                    .collect::<Vec<_>>(),
            )
            .with_metadata(std::collections::HashMap::from([
                (
        ...
        let batch = RecordBatch::try_new(
            schema,
            columns.into_iter().map(|(_, _, values)| values).collect(),
        )
        .unwrap();
        let staging =
            std::env::temp_dir().join(format!("roads-profile-retention-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&staging);
        super::super::write::stage(
            &staging,
            &[batch],
            &super::super::spatial::RoadIndex::new(Vec::new()),
        )
        .unwrap();
        ...
        assert_eq!(
            finalized[0]
                .schema()
                .metadata()
                .get(super::super::input::CONTRACT),
            Some(&"1".to_owned())
        );
        let profile_ids = finalized[0]
            ...
            .unwrap();
        assert!(
            profile_ids.iter().all(|v| v == Some(1)),
            "every child keeps the parent's profile reference"
        );
        let _ = std::fs::remove_dir_all(&staging);
    }
}
