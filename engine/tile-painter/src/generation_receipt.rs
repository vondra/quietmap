//! Published source/raster/code pins let a popup verify a different producer executable.
...
impl GenerationReceipt {
    pub fn generation(self) -> CornerGeneration {
        CornerGeneration::from_manifests(self.sources, self.rasters, self.code, self.producer)
    }
    pub fn publish(self, root: &Path) -> Result<()> {
        ...
        tx.execute(
            "INSERT OR IGNORE INTO surface_generation VALUES(1,?1,?2,?3,?4)",
            params![
                self.sources.as_slice(),
                self.rasters.as_slice(),
                self.code.as_slice(),
    ...
    pub fn read_compatible(
        ...
        let receipt = read_receipt(&connection)?;
            ...
            receipt.sources == sources
                && receipt.rasters == rasters
                && receipt.code == SURFACE_CODE_DIGEST,
            "stored corners do not match current sources, rasters and shared physics"
        );
        Ok(Some(receipt))
    }
}
fn read_receipt(connection: &Connection) -> Result<GenerationReceipt> {
    let values: [Vec<u8>; 4] = connection.query_row(
        "SELECT sources,rasters,code,producer FROM surface_generation WHERE id=1",
        [],
...
mod tests {
    ...
    fn reader_accepts_producer_receipt_only_with_current_code_and_external_pins() {
        ...
        assert!(GenerationReceipt::read_compatible(&root, [1; 32], [2; 32])
            .unwrap()
            .is_none());
        assert!(!root.exists());
        ...
        assert_eq!(
            GenerationReceipt::read_compatible(&root, [1; 32], [2; 32]).unwrap(),
            Some(receipt)
        );
        assert!(GenerationReceipt::read_compatible(&root, [5; 32], [2; 32]).is_err());
        assert!(GenerationReceipt::read_compatible(&root, [1; 32], [5; 32]).is_err());
        ...
        connection
            ...
            .unwrap();
        assert!(GenerationReceipt::read_compatible(&root, [1; 32], [2; 32]).is_err());
    }
}
