...
//!
//! `set_admin_h3r4_directory` fills a process-wide cache that the road and
...
use noise_compute::admin::{
    admin_for_hex, admin_for_latlng, cell_admin_path, set_admin_h3r4_directory, Continent,
};
use std::fs;

/// Dobříš (49.78 N, 14.17 E) — the R4 cell the benchmarks anchor on.
const DOBRIS: u64 = 0x0841_e309_ffff_ffff;

#[test]
fn recorded_directory_resolves_by_hex_and_by_latlng() {
    let tree = tempfile::tempdir().unwrap();
    let path = cell_admin_path(tree.path(), DOBRIS);
    fs::create_dir_all(path.parent().unwrap()).unwrap();
    let mut record = DOBRIS.to_le_bytes().to_vec();
    record.push(Continent::Europe as u8);
    record.extend_from_slice(b"CZ");
    record.extend_from_slice(&31u16.to_le_bytes());
    fs::write(&path, record).unwrap();

    set_admin_h3r4_directory(tree.path());

    assert_eq!(admin_for_hex(DOBRIS).country_code(), Some("CZ"));
    assert_eq!(admin_for_hex(DOBRIS).city_id, 31);
    // The same record through the popup's lat/lng entry point.
    assert_eq!(admin_for_latlng(49.78, 14.17).country_code(), Some("CZ"));
    // A cell the tree does not prepare (mid-Pacific) stays unknown.
