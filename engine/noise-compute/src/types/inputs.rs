...
pub struct RoadSegment {
    ...
    pub oneway: bool,
    pub lanes: u8,
    /// Prepared effective traffic from `roads-finalize` (Float64 counts +
    /// per-category estimated bitmask) — consumed verbatim by the kernel.
...
pub struct PointSource {
    ...
    /// Snapped z30 grid ring (empty = unavailable). Replaces the hex-era WKB
    /// hex string — same role (self-screening exclusion, popup display).
    pub polygon_grid: Vec<(i32, i32)>,
    ...
    pub floors: u8,
    /// Building footprint area (m²) computed at extract time from the snapped
    /// ring via the cos²(lat) Shoelace formula (`grid::poly::ring_area_m2`). 0 for
