...
/// `auto-<lonE5>-<latE5>` keys (integers = degrees × 1e5) surface as
/// coordinate display names instead of the opaque key.
#[test]
fn synth_auto_key_formats_coordinates() {
    assert_eq!(
        synth_airport_display_name("auto-1426000-5004000"),
        "Auto airfield 50.04,14.26"
    );
    // Negative coordinates on either side still split correctly.
    assert_eq!(
        synth_airport_display_name("auto--74250-4071000"),
        "Auto airfield 40.71,-0.74"
    );
    assert_eq!(
        synth_airport_display_name("auto-1426000--3350000"),
        "Auto airfield -33.50,14.26"
    );
    assert_eq!(
        synth_airport_display_name("auto--74250--3350000"),
        "Auto airfield -33.50,-0.74"
    );
}

/// Real airport keys, strip fallbacks, and malformed auto keys pass
/// through verbatim.
#[test]
fn non_auto_keys_pass_through_verbatim() {
    assert_eq!(synth_airport_display_name("LKPR"), "LKPR");
    assert_eq!(
        synth_airport_display_name("strip:z9/276/173"),
        "strip:z9/276/173"
    );
    assert_eq!(synth_airport_display_name("auto-"), "auto-");
