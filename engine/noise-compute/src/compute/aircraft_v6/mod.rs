...
mod tests {
    ...
    fn repeated_identical_clicks_are_bit_identical() {
        ...
        // `top_flight_candidates` both fill up. Plain lat/lon centroids on
        // an 11×12 lattice (~3 km spacing) around the receiver — no H3.
        let centroids: Vec<(f64, f64)> = (0..132)
            .map(|i| {
                let row = (i / 12) as f64;
                let col = (i % 12) as f64;
                (50.0 + (row - 5.0) * 0.03, 14.0 + (col - 5.5) * 0.045)
            })
            .collect();
        let cruise_cs: Vec<String> = (0..centroids.len()).map(|i| format!("DLH{i:04}")).collect();
        let cand_store: Vec<Vec<CruiseTopCandidateView<'_>>> = (0..centroids.len())
            .map(|i| {
            ...
            .collect();
        let cruise: Vec<CruiseRowView<'_>> = centroids
            .iter()
            .enumerate()
            .map(|(i, &(lat, lon))| CruiseRowView {
                lon,
                lat,
                class: 0,
