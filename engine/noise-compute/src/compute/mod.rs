...
/// regression-tested. Contributor accumulators here sum f64 energies across
/// a whole map, and f64 addition is not associative — so the iteration order
/// is part of the contributor result. The default `RandomState` re-seeds on
/// every `HashMap::new()`, i.e. on every popup query rather than merely every
/// process, which moved `total_lden` by ±1 ULP between repeats of one click
/// (measured 2026-08-05: three distinct values in four runs of the same Praha
/// click).
///
...
/// maps are small (one entry per road group / rail contributor / flight / point
