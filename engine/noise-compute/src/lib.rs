...
pub fn compute_at_point(
    ...
    // two chains are independent — own skyline, own trace list. Traces are
    // appended in the sequential order (roads, then railways), so the answer
    // is the sequential composition bit for bit. A scoped thread, not
    // `rayon::join`: a multi-second non-yielding chain must not sit on a
    // pool worker that pass 2 of every concurrent popup wants to steal from.
    // Rail stays on the calling thread, whose REACH_CACHE memo it fills.
