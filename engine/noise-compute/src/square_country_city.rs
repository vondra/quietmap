//! Continent, country and metro city of each prepared z9 square, read from `z9/x/y/square-country-city.bin`.
...
/// The square-country-city record's file name beside one z9 unit's Arrow files.
pub const SQUARE_COUNTRY_CITY_FILE_NAME: &str = "square-country-city.bin";

/// Six major continents. Hand-coded ids must match the square-country-city build script.
...
/// Continent, country and metro city of one z9 square (the `square-country-city.bin` record).
///
...
pub struct SquareCountryCity {
    pub continent: Continent,
    pub country_iso: [u8; 2],
    pub city_id: u16,
}

impl SquareCountryCity {
    pub const UNKNOWN: SquareCountryCity = SquareCountryCity {
...
/// The path of one square's square-country-city record inside a prepared year.
pub fn cell_square_country_city_path(
    ...
    Ok(prepared_directory
        ...
        .join(SQUARE_COUNTRY_CITY_FILE_NAME))
}

/// Read one square's square-country-city record. `io::ErrorKind::NotFound` means the square
...
pub fn read_cell_square_country_city(
    prepared_directory: &Path,
    square: i64,
) -> io::Result<SquareCountryCity> {
    let path = cell_square_country_city_path(prepared_directory, square)?;
...
#[derive(Default)]
struct SquareCountryCityCache {
    prepared_directory: Option<PathBuf>,
    by_square: HashMap<i64, SquareCountryCity>,
}

static SQUARE_COUNTRY_CITY_CACHE: LazyLock<RwLock<SquareCountryCityCache>> =
    LazyLock::new(RwLock::default);

const POISONED: &str = "a thread panicked while holding the square_country_city cache";

/// Point the process at the prepared squares tree the square-country-city records are read from.
/// Switching trees drops every cached record, so a re-initialised
/// source-reader can never answer from the tree it just left.
pub fn set_square_country_city_prepared_directory(prepared_directory: &Path) {
    let mut cache = SQUARE_COUNTRY_CITY_CACHE.write().expect(POISONED);
...
/// Resolve the SquareCountryCity for a given z9 z-order square id ([`square_id`]).
/// `SquareCountryCity::UNKNOWN` when no squares tree was recorded or the square is
/// outside the prepared world.
pub fn square_country_city_for_square(square: i64) -> SquareCountryCity {
    {
        let cache = SQUARE_COUNTRY_CITY_CACHE.read().expect(POISONED);
        if let Some(square_country_city) = cache.by_square.get(&square) {
            return *square_country_city;
        }
    ...
    let square_country_city = match read_cell_square_country_city(&prepared_directory, square) {
        Ok(square_country_city) => square_country_city,
        ...
        Err(error) if error.kind() == io::ErrorKind::NotFound => SquareCountryCity::UNKNOWN,
        Err(error) => panic!("{error}"),
    };
    cache.by_square.insert(square, square_country_city);
    square_country_city
}

/// Resolve the SquareCountryCity for a lat/lng by projecting to the enclosing z9 square.
/// Callers in source-reader (popup) have lat/lng from the HTTP request.
pub fn square_country_city_for_latlng(lat: f64, lng: f64) -> SquareCountryCity {
    square_country_city_for_square(square_id(grid::square_of(lat, lng)))
}
...
mod tests {
    ...
    fn missing_record_in_an_existing_square_directory_is_a_fault() {
        ...
        let error = read_cell_square_country_city(root, square).unwrap_err();
        assert_ne!(error.kind(), io::ErrorKind::NotFound, "{error}");
    }

    /// Write one square's square-country-city record into a squares tree.
    fn write_cell_square_country_city(
        prepared_directory: &Path,
        square: i64,
        iso: &[u8; 2],
        city: u16,
    ) {
        let path = cell_square_country_city_path(prepared_directory, square).unwrap();
    ...
    #[test]
    fn square_country_city_unknown_sentinel() {
        assert_eq!(SquareCountryCity::UNKNOWN.country_code(), None);
        assert_eq!(SquareCountryCity::UNKNOWN.continent, Continent::Unknown);
        assert_eq!(SquareCountryCity::UNKNOWN.city_id, 0);
    }

    #[test]
    fn country_code_roundtrips_ascii() {
        let a = SquareCountryCity {
    ...
    #[test]
    fn read_cell_square_country_city_returns_the_written_record() {
        let tree = tempfile::tempdir().unwrap();
        let dobris = square_id(grid::square_of(49.78, 14.17));
        write_cell_square_country_city(tree.path(), dobris, b"CZ", 31);
        let square_country_city = read_cell_square_country_city(tree.path(), dobris).unwrap();
    ...
    #[test]
    fn read_cell_square_country_city_rejects_a_record_from_another_square() {
        let tree = tempfile::tempdir().unwrap();
        let dobris = square_id(grid::square_of(49.78, 14.17));
        let neighbour = square_id(grid::Square { x: 0, y: 0 });
        write_cell_square_country_city(tree.path(), dobris, b"CZ", 0);
        fs::create_dir_all(tree.path().join("z9/0/0")).unwrap();
        fs::copy(
            cell_square_country_city_path(tree.path(), dobris).unwrap(),
            cell_square_country_city_path(tree.path(), neighbour).unwrap(),
        )
        .unwrap();
        let error = read_cell_square_country_city(tree.path(), neighbour).unwrap_err();
        assert_eq!(error.kind(), io::ErrorKind::InvalidData);
    }

    #[test]
    fn read_cell_square_country_city_reports_an_absent_square_as_not_found() {
        let tree = tempfile::tempdir().unwrap();
        let error =
            read_cell_square_country_city(tree.path(), square_id(grid::square_of(49.78, 14.17)))
                .unwrap_err();
        assert_eq!(error.kind(), io::ErrorKind::NotFound);
    }

    #[test]
    fn read_cell_square_country_city_rejects_invalid_ids_and_truncated_records() {
        let tree = tempfile::tempdir().unwrap();
        for square in [-1, grid::MAX_SQUARE_ID + 1] {
            assert_eq!(
                read_cell_square_country_city(tree.path(), square)
                    .unwrap_err()
                    .kind(),
                io::ErrorKind::InvalidInput
            );
        ...
        let square = square_id(grid::Square { x: 276, y: 173 });
        write_cell_square_country_city(tree.path(), square, b"CZ", 0);
        fs::write(
            cell_square_country_city_path(tree.path(), square).unwrap(),
            [0; RECORD_SIZE - 1],
        )
        .unwrap();
        assert_eq!(
    ...
    // The process-wide wiring test lives in tests/square_country_city_process_directory.rs —
    // its own process, so the lib tests keep their "no square-country-city tree is visible"
