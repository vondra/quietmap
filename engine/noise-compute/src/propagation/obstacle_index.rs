...
/// cells it sits in — a Prague click walked 4.5 M raw arcs and Paris 15 M for
/// well under a tenth that many distinct edges (2026-09-07) — and a repeat is an
/// exact no-op in the merge (the arc it would add already lies inside the
/// merged arc that absorbed it), so it is skipped before the bearing math.
/// A bit per edge rather than [`CrossingScratch`]'s lossy 64-slot tag table or
/// an epoch per edge: a skyline lives per worker and outlasts many receivers,
/// and clearing touches only the words a receiver set.
#[derive(Default)]
...
impl ObstacleIndex {
    ...
    fn visit_building_sector_crossings<const SECTORS: usize>(
        &self,
        receiver_lat: f64,
        receiver_lon: f64,
    ...
    pub fn crossings(
        ...
        self.walk(src_lat, src_lon, rcv_lat, rcv_lon, CellGate::All, out);
    }
    ...
    /// This index's crossings of the ray under `gate`, t-sorted into `out`.
    fn walk(
        &self,
        src_lat: f64,
        src_lon: f64,
    ...
    /// (the hot scatter loop runs this per receiver ray).
    fn append_crossings(
        &self,
        src_lat: f64,
        src_lon: f64,
...
impl ObstacleSet {
    ...
    pub fn crossings_pruned(
        ...
        self.crossings_pruned_with_scratch(
            src_lat,
            src_lon,
            rcv_lat,
            rcv_lon,
...
mod tests {
    ...
    fn max_height_crossed_reads_exact_crossings() {
        ...
        assert_eq!(probe(OLAT, OLON + 0.02), 12.0);
