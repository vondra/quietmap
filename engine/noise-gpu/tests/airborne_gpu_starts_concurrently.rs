...
//! and the prep thread blocked forever on the depth-1 channel. Five rounds, because that
//! is the sample the failure was measured over.
//!
    ...
    for round in 0..ROUNDS {
        std::thread::scope(|scope| {
            let workers: Vec<_> = (0..STREAM_WORKERS)
                .map(|_| scope.spawn(|| AirborneGpu::new(&class_weights).vram_total_bytes()))
                .collect();
