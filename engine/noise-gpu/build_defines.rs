...
///
/// The list itself lives in `scripts/reviewed-defines.txt` so the Rust build gate
/// and the Python role validator cannot drift apart -- they did, on the day the
/// terrain arm was added to one copy and not the other. It sits beside
/// `gpu_model_role.py` because every selftest harness that stages the validator
/// copies `scripts/`, and a file anywhere else is one those harnesses forget.
const REVIEWED_EXPERIMENTAL_DEFINES_RAW: &str = include_str!("../../scripts/reviewed-defines.txt");

fn reviewed_experimental_defines() -> Vec<&'static str> {
    REVIEWED_EXPERIMENTAL_DEFINES_RAW
        .lines()
        .map(str::trim)
...
pub(crate) fn parse_experimental_defines(raw: &str) -> Result<Vec<String>, String> {
    ...
    for token in raw.split_ascii_whitespace() {
        ...
        if !reviewed_experimental_defines().contains(&name) {
            return Err(format!(
