...
/** The hermetic world's published zoom (a z12 world, while production paints
 *  z13: the frontend must take its tile ceiling from the manifest). */
...
export async function installHermeticMap(
  ...
  // A click or fly-to arrival opens the point popup; tests that assert it
  // register their own routes later (later routes win), the rest stay
  // hermetic with an explicit 404 instead of the preview server's fallback.
  for (const api of ['**/api/noise-onfly-v2?**', '**/api/noise-preview?**']) {
