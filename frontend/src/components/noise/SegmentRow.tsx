...
function highlightGeometry(t: SegmentTrace) {
  ...
  //   CRUISE = cell_polygon → Polygon (ring already closed by backend)
  if (t.kind === 'aircraft') {
    ...
    if (t.aircraft_subtype === AIRCRAFT_SUBTYPE.CRUISE && t.cell_polygon && t.cell_polygon.length >= 4) {
      return { type: 'Polygon', coordinates: [t.cell_polygon.map(flipLatLon)] }
    }
  }
