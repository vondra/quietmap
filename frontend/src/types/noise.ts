...
interface RoadMetadata {
  ...
  speed_source:
    ...
    | 'roundabout_cap'
    | 'derestricted'
    // Taper-graded effective speed at a junction-free step — a refinement
...
export interface AircraftTopFlight {
   ...
   *  cruise-cell bucket aggregates; UI hides ICAO + time for these. */
  synthetic: boolean
}
...
// (ground path / airborne sub-segment / cruise cell) are split via
 ...
 * sub-segment / cruise cell. Each is capped separately by the
