...
// a deeper floor would blank e.g. roads-only at z4). The TOP of the band is
// not a constant: it is the zoom the published world was painted at,
// delivered per manifest as `TileBuilds.zoom`.
 ...
 *  OWN build — a partial republish (e.g. road-only b547 over a b546 world)
 *  flips only that layer's URLs, and the untouched layers re-fetch straight
 *  from the browser's immutable cache. */
...
async function refreshTileBuild(): Promise<void> {
  try {
    ...
    const manifest = (await res.json()) as {
      ...
      zoom?: unknown
      tile_base?: unknown
      layers?: Record<string, { build?: unknown }>
    }
    ...
    for (const [layer, entry] of Object.entries(manifest.layers ?? {})) {
      byLayer[layer] =
        typeof entry?.build === 'string' && BUILD_ID.test(entry.build) ? entry.build : manifest.build
    }
