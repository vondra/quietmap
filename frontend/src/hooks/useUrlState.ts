...
function parseHash(): UrlState {
  ...
  // (advanced rasters off). An explicit empty `ro=` therefore means "all off".
...
function buildHash(state: {
  ...
  if (state.rasterOverlays) {
    ...
    // Omit `ro` for the default view (all advanced rasters off) so a bare
    // link opens the default; serialize the exact set otherwise (including
